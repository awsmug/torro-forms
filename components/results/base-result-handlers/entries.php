...
class AF_ResultsEntries extends AF_ResultHandler
{
	...
	public function option_content()
	{
		...
		if( $form_results->count() > 0 )
		{
			...
			$html .= '<th class="export-links">' . sprintf( __( 'Export as <a href="%s">XLS</a> or <a href="%s">CSV</a>', 'af-locale' ), admin_url( 'edit.php' ) . '?post_type=questions&export=xls&form_id=' . $form_id, admin_url( 'edit.php' ) . '?post_type=questions&export=csv&form_id=' . $form_id ). '</th>';
			$html .= '</tr>';
			$html .= '</thead>';
