...
/**
 * Class for a field mappings field.
 *
 * @since 1.1.0
 */
class Field_Mappings_Field extends Field {
	...
	protected function render_single_input( $current_value ) {
		...
		?>
		<div id="<?php echo esc_attr( $id ); ?>"<?php echo $class; /* phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped */ ?>>
			...
			<?php foreach ( $this->params as $param_slug => $param_info ) : ?>
				...
				<div id="<?php echo esc_attr( $id . '-' . $param_slug . '-wrap' ); ?>" class="field-mappings-param-wrap">
					...
					<?php elseif ( 'boolean' === $param_info['type'] ) : ?>
						<input type="checkbox" id="<?php echo esc_attr( $id . '-' . $param_slug ); ?>" name="<?php echo esc_attr( $id . '[' . $param_slug . ']' ); ?>" class="field-mappings-param-control"<?php echo $param_value ? ' checked' : ''; ?>>
					<?php elseif ( 'integer' === $param_info['type'] ) : ?>
						<input type="number" id="<?php echo esc_attr( $id . '-' . $param_slug ); ?>" name="<?php echo esc_attr( $id . '[' . $param_slug . ']' ); ?>" class="field-mappings-param-control" value="<?php echo esc_attr( $param_value ); ?>"<?php echo ! empty( $param_info['minimum'] ) ? ' min="' . esc_attr( $param_info['minimum'] ) . '"' : ''; ?><?php echo ! empty( $param_info['maximum'] ) ? ' max="' . esc_attr( $param_info['maximum'] ) . '"' : ''; ?> step="1">
					<?php elseif ( 'float' === $param_info['type'] || 'number' === $param_info['type'] ) : ?>
						<input type="number" id="<?php echo esc_attr( $id . '-' . $param_slug ); ?>" name="<?php echo esc_attr( $id . '[' . $param_slug . ']' ); ?>" class="field-mappings-param-control" value="<?php echo esc_attr( $param_value ); ?>">
					<?php elseif ( 'string' === $param_info['type'] && ! empty( $param_info['enum'] ) ) : ?>
						<select id="<?php echo esc_attr( $id . '-' . $param_slug ); ?>" name="<?php echo esc_attr( $id . '[' . $param_slug . ']' ); ?>" class="field-mappings-param-control">
					...
					<?php else : ?>
						<input type="text" id="<?php echo esc_attr( $id . '-' . $param_slug ); ?>" name="<?php echo esc_attr( $id . '[' . $param_slug . ']' ); ?>" class="field-mappings-param-control" value="<?php echo esc_attr( $param_value ); ?>">
					<?php endif; ?>
				</div>
			<?php endforeach; ?>
		</div>
			...
			<# _.each( data.params, function( paramInfo, paramSlug ) { #>
				...
				<div id="{{ data.id + '-' + paramSlug + '-wrap' }}" class="field-mappings-param-wrap">
					...
					<# } else if ( 'boolean' === paramInfo.type ) { #>
						<input type="checkbox" id="{{ data.id + '-' + paramSlug }}" name="{{ data.id + '[' + paramSlug + ']' }}" class="field-mappings-param-control" value="1"<# if ( paramValue ) { #> checked<# } #>>
					<# } else if ( 'integer' === paramInfo.type ) { #>
						<input type="number" id="{{ data.id + '-' + paramSlug }}" name="{{ data.id + '[' + paramSlug + ']' }}" class="field-mappings-param-control" value="{{ paramValue }}"<# if ( paramInfo.minimum ) { #> min="{{ paramInfo.minimum }}"<# } #><# if ( paramInfo.maximum ) { #> max="{{ paramInfo.maximum }}"<# } #> step="1">
					<# } else if ( 'float' === paramInfo.type || 'number' === paramInfo.type ) { #>
						<input type="number" id="{{ data.id + '-' + paramSlug }}" name="{{ data.id + '[' + paramSlug + ']' }}" class="field-mappings-param-control" value="{{ paramValue }}">
					<# } else if ( 'string' === paramInfo.type && paramInfo.enum ) { #>
						<select id="{{ data.id + '-' + paramSlug }}" name="{{ data.id + '[' + paramSlug + ']' }}" class="field-mappings-param-control">
							<# _.each( paramInfo.enum, function( enumValue ) { #>
								<option value="{{ enumValue }}"<# if ( paramValue === String( enumValue ) ) { #> selected<# } #>>{{ enumValue }}</option>
							<# } ) #>
						</select>
					<# } else { #>
						<input type="text" id="{{ data.id + '-' + paramSlug }}" name="{{ data.id + '[' + paramSlug + ']' }}" class="field-mappings-param-control" value="{{ paramValue }}">
					<# } #>
				</div>
			<# } ) #>
		</div>
